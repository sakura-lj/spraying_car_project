...
- `publish_tf=false`，默认不发布 `odom -> base_link`。
...
L1RM、point_lio 实时运行，`point_lio_frame_bridge` 输出 `/slam_odom`。在实测确认后，可以开启 `odom -> base_link`，用于低速局部导航实验。
...
阶段 8A 不发布 `map -> odom`。`odom -> base_link` 也默认关闭。
...
进入 move_base/TEB dry-run 前，live SLAM 模式需要 `odom -> base_link`。只有在实测确认 `/slam_odom` 方向和 `base_link -> lidar_link` 外参正确后，才应使用：
