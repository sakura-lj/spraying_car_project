...
2. **UART (upper computer)** — USART1 @ 115200, DMA + idle-line RX. `upper.c` runs a byte-wise state machine parsing framed packets `[0xAA][type][len][data...][checksum=Σdata][0x55]`. Command types are `CMD_SPRAY_CONTROL=0x01`, `CMD_SPEED_CONTROL=0x02`, `CMD_DIRECTION_CONTROL=0x03`, `CMD_TURN_CONTROL=0x04`, `CMD_STATUS_QUERY=0xFF` (responds with legacy `CMD_STATUS_RESPONSE=0x05`) and `CMD_EXT_STATUS_QUERY=0x06` (responds with extended `CMD_EXT_STATUS_RESPONSE=0x07`). `packData()` / `sendData()` build replies; `send_status_data()` must keep the old 4-byte shape, while `send_ext_status_data()` adds the 26-byte little-endian extended status.
...
- **Keep legacy and extended status side by side**. Do not replace `CMD_STATUS_RESPONSE=0x05`; old Web clients depend on `[spray_state, speed_duty, direction, is_open]`. Extended status `0x07` includes `turn_cmd_position`, `turn_target_encoder`, `turn_encoder_position`, `uart_control_mode`, plus placeholder `battery_mv=0`, `fault_code=0`, `safety_state=0`.
